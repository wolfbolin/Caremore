...
def serial_service(serial_dict):
    ...
    if sim_serial.isOpen():
        ...
        sim_serial.write("ATE0\r\n".encode("utf-8"))
        sim_serial.write("AT+CLCC=1\r\n".encode("utf-8"))
        sim_serial.write("AT+CMGF=1\r\n".encode("utf-8"))
        sim_serial.write("AT+CHFA=1\r\n".encode("utf-8"))
        sim_serial.write("ATM6\r\n".encode("utf-8"))
        sim_serial.write("ATL6\r\n".encode("utf-8"))
    ...
    try:
        ...
        print("[INFO] Can't open serial thread.")


def uno_translator(uno_serial, serial_dict):
    print('[INFO] UNO ready to work')
    while True:
        ...
        if count != 0:
            ...
            print('[INFO] Receive from UNO:', recv)
...
def sim_translator(sim_serial, serial_dict):
    ...
    while True:
        ...
        if count != 0:
            ...
            recv = recv.decode().rstrip()
            print('[INFO] Receive from SIM:', recv)
            # Some code to analysis
            if recv == 'OK':
                continue
            elif recv == 'RING':
